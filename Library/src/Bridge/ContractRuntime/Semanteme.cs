...
{
    /// <summary>
    /// Base node for the TOMB asm parser pipeline (label/instruction).
    /// TOMB emits asm text and assembles it deterministically through this pipeline.
    /// </summary>
    public abstract class Semanteme
    {
        ...
        {
            // Alias state must be reset per script to avoid cross-script pollution.
            ...
            {
                // Keep original line index for debug range reconstruction.
                ...
                if (index >= 0)
                {
                    // Labels can share a line with an instruction; emit label first.
